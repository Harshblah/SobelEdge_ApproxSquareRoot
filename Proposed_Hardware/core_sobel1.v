module core_sobel1 (
    ...
    generate
        for (k = 0; k < 5; k = k + 1) begin : predict_k
            ...
            k_predictor pred1 (.key(key[k]), .k_value(k_value[k]));
        end
    endgenerate
    ...
    squareroot_MAHSQR_k4 sqrt_k4_1 (.R(R_selected_k4), .final_op(out_k4));
    squareroot_MAHSQR_k6 sqrt_k6_1 (.R(R_selected_k6), .final_op(out_k6));
    squareroot_MAHSQR_k8 sqrt_k8_1 (.R(R_selected_k8), .final_op(out_k8));
    squareroot_MAHSQR_k10 sqrt_k10_1 (.R(R_selected_k10), .final_op(out_k10));
    squareroot_MAHSQR_k12 sqrt_k12_1 (.R(R_selected_k12), .final_op(out_k12));
